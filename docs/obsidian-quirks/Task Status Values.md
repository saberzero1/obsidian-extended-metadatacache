...
[^dataview-task]: [Dataview `markdown-file.ts:248-253` — task status parsing](https://github.com/blacksmithgu/obsidian-dataview/blob/5ad0994ff384/src/data-import/markdown-file.ts#L248-L253) distinguishes `checked` (any non-space) from `completed` (only `x`/`X`).
