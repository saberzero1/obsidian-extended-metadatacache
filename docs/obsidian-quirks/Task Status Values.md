...
[^dataview-task]: [Dataview markdown-file.ts — task status parsing](https://github.com/blacksmithgu/obsidian-dataview/blob/5ad0994ff384/src/data-import/markdown-file.ts) distinguishes `checked` (any non-space) from `completed` (only `x`/`X`).
